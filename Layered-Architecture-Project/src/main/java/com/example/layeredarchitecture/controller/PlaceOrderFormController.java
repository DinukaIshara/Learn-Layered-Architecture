...
//                    txtQtyOnHand.setText(tblOrderDetails.getItems().stream().filter(detail-> detail.getCode().equals(item.getCode())).<Integer>map(detail-> item.getQtyOnHand() - detail.getQty()).findFirst().orElse(item.getQtyOnHand()) + "");
    ...
    private void loadAllCustomerIds() {
        try {
            ...
            ArrayList<String> ids = customerDAO.getAllCustomerIds();
    ...
    public void btnPlaceOrder_OnAction(ActionEvent actionEvent) {
        boolean b = false;
        try {
            b = placeOrderDAO.saveOrder(orderId, LocalDate.now(), cmbCustomerId.getValue(),
                    tblOrderDetails.getItems().stream().map(tm -> new OrderDetailDTO(tm.getCode(), tm.getUnitPrice(), tm.getQty())).collect(Collectors.toList()));
        } catch (SQLException e) {
